	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld", (long)seekByteOffset, fileLength - (long)1]);
			discontinuous = vbr;
		}
...
- (void) setVolume:(float)level
{
    ...
    if (errorMsg)
    {
        NSLog(@"AudioQueueSetParameter mengembalikan %ld ketika menyetel volume.", errorMsg);
    }
}
				...
				if (statusCode == 200)		// "OK" (this is true even for ICY)
				{
					...
					else
					{
						...
						if (serverHeader != nil && NSEqualRanges([serverHeader rangeOfString:@"Nanocaster"], NSMakeRange(0, 10))) {
                            ...
                            // eka chan 20130612 - comment contentType and icyBr
...
- (void)createQueue
{
    ...
    UInt32 val = kAudioQueueHardwareCodecPolicy_PreferSoftware;         // eka chan 20130710
	...
	ioFlags:(UInt32 *)ioFlags
{
	@synchronized(self)
	{
		...
		else if (inPropertyID == kAudioFileStreamProperty_FormatList)
		{
			...
			{
				[self failWithErrorCode:AS_FILE_STREAM_GET_PROPERTY_FAILED];
                // ekachan 20130612 ... add free(formatList);
                free(formatList);
				return;
			}
